...
<head>
	...
	<link rel="stylesheet" href="${appPublic}/Backstage/css/order.css">
...
<div class="wrapper">
	<div class="main">
		...
		<div class="content">
			<div class="title"><a href="${appRoot}/Backstage/doctor">医生管理</a> > 医疗记录 > ${doctorName}</div>
			<div class="table">
				<table>
					<thead>
						<tr>
			...
			<div class="page">
                <c:if test="${page > 0}">
                    <a class="icon-chevron-left" href="${appRoot}/Backstage/order?doctorId=${doctorId}&doctorName=${doctorName}&page=${page-1}"></a>
                </c:if>
                <c:forEach var="item" varStatus="status" begin="0" end="${size}">
                    <c:choose>
                        <c:when test="${status.index == page}">
                            <a class="active" href="${appRoot}/Backstage/order?doctorId=${doctorId}&doctorName=${doctorName}&page=${status.index}">${status.index+1}</a>
                        </c:when>
                        <c:otherwise>
                            <a href="${appRoot}/Backstage/order?doctorId=${doctorId}&doctorName=${doctorName}&page=${status.index}">${status.index+1}</a>
                        </c:otherwise>
                    </c:choose>
                </c:forEach>
                <c:if test="${page < size}">
                    <a class="icon-chevron-right" href="${appRoot}/Backstage/order?doctorId=${doctorId}&doctorName=${doctorName}&page=${page+1}"></a>
                </c:if>
				<div class="search">
					<span>跳至</span>
					<select class="searchPage" name="search" data-user="&doctorId=${doctorId}&doctorName=${doctorName}">
                        <option value=""></option>
                        <c:forEach var="item" varStatus="status" begin="0" end="${size}">
...
<script src="${appPublic}/Backstage/js/order.js"></script>
</body>
</html>