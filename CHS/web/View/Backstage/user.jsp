...
<head>
	...
	<link rel="stylesheet" href="${appPublic}/Backstage/css/user.css">
...
<div class="wrapper">
	<div class="main">
		...
		<div class="content">
			<div class="title"><a href="${appRoot}/Backstage/user">用户管理</a></div>
			<div class="table">
				<table>
					<thead>
						<tr>
					...
					<tbody>
						<c:forEach items="${users}" var="user">
							<tr>
								<td><img class="table-head" src="${appRoot}${user.head}"/></td>
								<td>${user.name}</td>
								<td>${user.loginName}</td>
								<td>${user.createAt}</td>
								<td><a class="table-href" href="${appRoot}/Backstage/archives?userId=${user.id}&userName=${user.name}">查看</a></td>
								<td><a class="table-href" href="${appRoot}/Backstage/monitor?userId=${user.id}&userName=${user.name}">查看</a></td>
			...
			<div class="page">
                <c:if test="${page > 0}">
                    <a class="icon-chevron-left" href="${appRoot}/Backstage/user?page=${page-1}"></a>
                </c:if>
                <c:forEach var="item" varStatus="status" begin="0" end="${size}">
                    <c:choose>
                        <c:when test="${status.index == page}">
                            <a class="active" href="${appRoot}/Backstage/user?page=${status.index}">${status.index+1}</a>
                        </c:when>
                        <c:otherwise>
                            <a href="${appRoot}/Backstage/user?page=${status.index}">${status.index+1}</a>
                        </c:otherwise>
                    </c:choose>
                </c:forEach>
                <c:if test="${page < size}">
                    <a class="icon-chevron-right" href="${appRoot}/Backstage/user?page=${page+1}"></a>
                </c:if>
				<div class="search">
					<span>跳至</span>
...
<script src="${appPublic}/Backstage/js/user.js"></script>
</body>
</html>